...
function StepResult({ form, onRestart, onAddLetter }) {
  ...
  const downloadPDF = async () => {
    ...
    try {
      ...
      // Proposer PWA après téléchargement réussi
      setTimeout(() => {
        if (pwaPrompt && !pwaInstalled) setShowPwaBanner(true);
      }, 2000);
    }
  };
    ...
    <div>
      ...
      {showCongrats && (
        <div style={{ position:"fixed", top:80, left:"50%", transform:"translateX(-50%)",
          ...
          pointerEvents:"none" }}>
          <div style={{ background:"rgba(28,24,18,0.92)", WebkitBackdropFilter:"blur(", backdropFilter:"blur(12px)",
...
function LandingPage({ onStartCV, onStartLetter }) {
  ...
  const acceptCGU = () => {
    ...
    // Proposer PWA 5 secondes après acceptation CGU (2ème visite+)
    if (visitCount >= 2) {
      setTimeout(() => {
        if (pwaPrompt && !pwaInstalled) setShowPwaBanner(true);
      }, 5000);
      ...
      <div style={{ maxWidth:700, margin:"0 auto", padding:"clamp(28px,6vw,48px) var(--pad-x)" }}>
        ...
        <div style={{ textAlign:"center", padding:"20px 0", borderTop:"1px solid var(--border)" }}>
          <div style={{ display:"flex", justifyContent:"center", alignItems:"center", gap:8, marginBottom:8 }}>
            <Logo size={16}/>
            <span style={{ fontFamily:"'Playfair Display',serif", fontSize:12, fontStyle:"italic", color:"var(--ink4)", cursor:"pointer", userSelect:"none", WebkitUserSelect:"none" }}
...
export default function App() {
  ...
  const visitCount = (() => {
    try {
      const n = parseInt(localStorage.getItem("cvtools_visits")||"0") + 1;
      localStorage.setItem("cvtools_visits", String(n));
      return n;
    } catch(e) { return 1; }
  })();
  ...
  useEffect(() => {
    const handler = (e) => {
      e.preventDefault();
      setPwaPrompt(e);
      // À la 3ème visite ou plus → proposer automatiquement
      if (visitCount >= 3) {
  ...
  return (
    <>
      ...
      <div key={`app-${screenKey}`} className={`${darkMode?"dark":""} ${screenDir==="backward"?"screen-backward":"screen-forward"}`} style={{ background:"var(--cream)", minHeight:"100vh" }}>
        ...
        {showLeaveWarn && (
          <div style={{ position:"fixed", inset:0, background:"rgba(28,24,18,0.6)",
            WebkitBackdropFilter:"blur(", backdropFilter:"blur(4px)", WebkitBackdropFilter:"blur(4px)",
      ...
      {/* ── Bannière PWA ── */}
      {showPwaBanner && pwaPrompt && !pwaInstalled && (
        <div style={{
          ...
          <div style={{
            ...
            {/* Icône */}
            <div style={{ width:44, height:44, background:"var(--terra)",
            ...
            <div style={{ display:"flex", flexDirection:"column", gap:6, flexShrink:0 }}>
              <button onClick={async () => {
                ...
                style={{ background:"var(--terra)", color:"#fff", border:"none",
                  ...
                  boxShadow:"0 2px 8px rgba(184,92,56,0.4)", whiteSpace:"nowrap" }}>
                Installer
              </button>
              <button onClick={() => setShowPwaBanner(false)}
                style={{ background:"transparent", color:"rgba(255,255,255,0.4)",
                  border:"none", padding:"4px", fontSize:11, cursor:"pointer",
      ...
      {toast && (
        <div className="toast" style={{
          ...
          zIndex:9999, background:"rgba(28,24,18,0.93)", WebkitBackdropFilter:"blur(", backdropFilter:"blur(10px)",
