...
// Lazy loading — chargés à la demande
...
// Loader minimal affiché pendant le chargement d'un chunk
...
// Exposer sur window pour Admin.jsx (lazy loaded)
if (typeof window !== "undefined") {
  window.__SHEETS_URL     = SHEETS_URL;
  window.__sheetsGet      = (...a) => sheetsGet(...a);
  window.__sheetsPost     = (...a) => sheetsPost(...a);
  window.__sheetsPostResp = (...a) => sheetsPostResp(...a);
  window.__getAdminPwd    = (...a) => getAdminPwd(...a);
  window.__getAnnounce    = (...a) => getAnnounce(...a);
  window.__updateConfig   = (...a) => updateConfig(...a);
}
...
// ── Letter + CV renderers — chargés en lazy ──
// Ces composants sont dans TemplatesCV.jsx et TemplatesLetter.jsx
// Ils sont injectés via window après le lazy load

let _RENDERERS = null;
let _ATS_RENDERERS = null;
let _LetterRenderer = null;

function getRenderers() { return _RENDERERS || {}; }
function getATSRenderers() { return _ATS_RENDERERS || {}; }
function getLetterRenderer() { return _LetterRenderer; }

// Wrappers qui attendent le lazy load
function CVRendererWrapper({ cv }) {
  const [loaded, setLoaded] = React.useState(!!_RENDERERS);
  React.useEffect(() => {
    if (!_RENDERERS) {
      import("./TemplatesCV.jsx").then(m => {
        _RENDERERS = m.RENDERERS;
        _ATS_RENDERERS = m.ATS_RENDERERS;
        window.__CVFooter = m.CVFooter;
        setLoaded(true);
      });
    }
  }, []);
  if (!loaded) return <ChunkLoader/>;
  const Comp = (_RENDERERS[cv.template] || _ATS_RENDERERS[cv.template] || _RENDERERS.executive);
  return <Comp cv={cv}/>;
}

function LetterRendererWrapper({ cv }) {
  const [loaded, setLoaded] = React.useState(!!_LetterRenderer);
  React.useEffect(() => {
    if (!_LetterRenderer) {
      import("./TemplatesLetter.jsx").then(m => {
        _LetterRenderer = m.LetterRenderer;
        setLoaded(true);
      });
    }
  }, []);
  if (!loaded) return <ChunkLoader/>;
  return <_LetterRenderer cv={cv}/>;
}
...
function StepResult({ form, onRestart, onAddLetter }) {
  ...
  // Renderer lazy — CVRendererWrapper gère le chargement
            ...
            <div style={{ background:"#e8e0d0", padding:10, borderRadius:8,
              ...
              <div style={{ width: activeTab==="both" ? "100%" : 595*0.75,
                ...
                <div ref={letterRef} style={{
                  ...
                  transform: activeTab==="both" ? "scale(0.42)" : "scale(0.75)",
                  boxShadow:"0 4px 32px rgba(0,0,0,0.18)" }}>
                  <LetterRendererWrapper cv={letterCv}/>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="cv-viewer" style={{ background:"#e8e0d0", padding:12, borderRadius:8, overflowX:"auto", WebkitOverflowScrolling:"touch" }}>
        <div style={{ width:595*zoom, margin:"0 auto", transition:"width 0.2s", minWidth:"min-content" }}>
          <div ref={letterRef} style={{ width:595, transformOrigin:"top left",
            ...
            <LetterRendererWrapper cv={cvFinal}/>
          </div>
        </div>
      </div>
      ...
      {!cguDone && (
        <div style={{
          ...
          <button
            onClick={acceptCGU}
            style={{
              background: cguChecked ? "var(--terra)" : "rgba(184,92,56,0.35)",
    ...
    return (
      <>
        ...
        <Suspense fallback={<ChunkLoader/>}>
          <AdminDashboardLazy onBack={() => setScreen("landing")}/>
        </Suspense>
      </>
    );
  }
